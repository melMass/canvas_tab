...
function mirrorX(strokes) {
  return [
    ...strokes,
    ...strokes.map((a) => a.map(({ x, y }) => ({ x: 1 - x, y }))),
  ]
}
function mirrorY(strokes) {
  return [
    ...strokes,
    ...strokes.map((a) => a.map(({ x, y }) => ({ x, y: 1 - y }))),
  ]
}
...
function mirrorDiagonalXY(strokes) {
  return [
    ...strokes,
    ...strokes.map((a) => a.map(({ x, y }) => ({ x: y, y: x }))),
  ]
}
...
function mirrorDiagonalY1MinusX(strokes) {
  return [
    ...strokes,
    ...strokes.map((a) => a.map(({ x, y }) => ({ x: 1 - y, y: 1 - x }))),
  ]
}
...
function rotationalSymmetry(ways) {
  ...
  return function (strokes) {
    ...
    for (let i = 1; i < ways; i++) {
      ...
      const rotatedStrokes = strokes.map((a) =>
        a.map(({ x, y }) => ({
...
function spiralImage(fins, size = 24) {
  ...
  for (let i = 0; i < fins; i++) {
    const a = i * angularSpacing + 0.5

    const x = Math.sin(a) * size
    const y = Math.cos(a) * size
    const x2 = Math.sin(a - angularSpacing * 0.5) * size * 0.5
    const y2 = Math.cos(a - angularSpacing * 0.5) * size * 0.5
    ctx.beginPath()
